...
class Tile {
  ...
  detach() {
    if (this.isAttached) {
      ...
      if (
        this.tileImage.imageElement &&
        this.tileImage.imageElement.parentNode
      ) {
  ...
  _addToDOM() {
    ...
    if (!fadeInDuration || this.tiledLayer.isLowRes || this.isPlaceholder) {
    ...
    this.tileImage.imageElement.style.transition =
      "opacity " + fadeInDuration + "ms linear";
    this.tiledLayer.addTileToDOMWithRaf(this, () => {
...
class TileImagesCache {
  ...
  add(tileImage) {
    ...
    if (this._items.length > this.limit) {
      ...
      const indexToRemove = this._items.findIndex(
        (item) => !item.hasParent && !item.isLowRes
      );
      if (indexToRemove !== -1) {
  ...
  refresh() {
    ...
    this.images = this.images.filter((image) => {
      ...
      if (
        ...
        image.decodingState === DECODING_STATE.DECODED
      ) {
...
class TiledLayer {
  ...
  updateTilesVisibility(isLowRes) {
    ...
    tileCoordinatesToAttach.forEach((coordinate) => {
      let tile = this.tiler.manager.getOrCreateTile(
        coordinate.x,
        coordinate.y,
        this.z
      );
      ...
      if (!isLowRes) {
        if (!tile.canBeDisplayed() || !tile.isFullyDisplayed) {
          ...
          this.tiler.manager.showPlaceholderTilesAbove(
            tile.x,
            tile.y,
            tile.z,
            this.z + 1
  ...
  getTileCoordinatesInViewport() {
    ...
    const scale =
      slide.currZoomLevel /
      (slide.currentResolution || slide.zoomLevels.initial);

    const tileWidth =
      this.tiler.getBaseTileHeight(this.z) * this.tileScale * scale;
    ...
    const leftTileX = getTileCoordinateByPosition(
      -tileLeft,
      tileWidth,
      this.numXTiles
    );
    const rightTileX = getTileCoordinateByPosition(
      viewportRight - tileLeft,
      tileWidth,
      this.numXTiles
    );

    const topTileY = getTileCoordinateByPosition(
      -tileTop,
      tileHeight,
      this.numYTiles
    );
    const bottomTileY = getTileCoordinateByPosition(
      viewportBottom - tileTop,
      tileHeight,
      this.numYTiles
    );
...
class Tiler {
  constructor(slide, options) {
    ...
    if (this.options.maxTilePixelRatio > 1 && window.devicePixelRatio > 1) {
      this.tilePixelRatio = Math.min(
        window.devicePixelRatio,
        this.options.maxTilePixelRatio
      );
    } else {
  ...
  setupDeepzoomLayers() {
    this.minZoomLayer = 0;
    this.maxZoomLayer = Math.ceil(
      Math.log(Math.max(this.maxWidth, this.maxHeight)) / Math.log(2)
    );
  }
  ...
  updatePrimaryImageVisibility() {
    if (this.slide.primaryImageWidth && this.width) {
      // Do not show tiles if image is smaller than "fit" zoom level
      if (
        this.width <=
        Math.round(this.pswp.currSlide.zoomLevels.fit * this.maxWidth)
      ) {
        return true;
      }
  ...
  updateSize() {
    ...
    if (useLowResLayer) {
      lowResLayer = this.layers.find((layer) => {
        return (
          layer.originalWidth >= this.tileSize ||
          layer.originalHeight >= this.tileSize
        );
      });
    }
  ...
  getZoomifyTileUrl(x, y, z) {
    ...
    for (let i = 0; i < z; i++) {
      tileNumber +=
        this._zoomifyLayers[i].xTilesCount * this._zoomifyLayers[i].yTilesCount;
    }
...
class DeepZoomUI {
  ...
  addButtons() {
    this.pswp.ui.registerElement({
      ...
      html: {
        isCustomSVG: true,
        inner:
          '<path d="M17.426 19.926a6 6 0 1 1 1.5-1.5L23 22.5 21.5 24l-4.074-4.074z" id="pswp__icn-incremental-zoom-in"/>' +
    ...
    this.pswp.ui.registerElement({
      ...
      html: {
        isCustomSVG: true,
        inner:
          '<path d="M17.426 19.926a6 6 0 1 1 1.5-1.5L23 22.5 21.5 24l-4.074-4.074z" id="pswp__icn-incremental-zoom-out"/>' +
          '<path fill="currentColor" d="M11 16v-2h6v2z"/>',
    ...
    this.pswp.ui.registerElement({
      ...
      html: {
        isCustomSVG: true,
        inner:
          '<path d="M11.213 9.587 9.961 7.91l-1.852 5.794 6.082-.127-1.302-1.744a5.201 5.201 0 0 1 7.614 6.768 5.2 5.2 0 0 1-7.103 1.903L12 22.928a8 8 0 1 0-.787-13.34Z" id="pswp__icn-zoom-to-start"/>',
  ...
  incrementalZoomIn(point) {
    ...
    if (tiler) {
      ...
      destZoomLevel = Math.min(
        destZoomLevel,
        this.pswp.currSlide.zoomLevels.secondary
      );
    } else {
  ...
  incrementalZoomOut(point) {
    ...
    if (tiler) {
      ...
      destZoomLevel = Math.max(
        destZoomLevel,
        this.pswp.currSlide.zoomLevels.initial
      );
    } else {
  ...
  updateZoomInButtonState(el) {
    if (
      !this.pswp.currSlide.currZoomLevel ||
  ...
  updateZoomToStartButtonState(el) {
    if (
      ...
      !this.pswp.currSlide.isZoomable() ||
      this.pswp.currSlide.currZoomLevel <=
        this.pswp.currSlide.zoomLevels.initial * 3
    ) {
      el.setAttribute("disabled", "disabled");
      ...
      if (e.slideData.tileUrl) {
        ...
        if (e.slideData.maxZoomWidth) {
          ...
          if (maxWidth) {
            ...
            e.zoomLevels.max = Math.max(e.zoomLevels.initial, newMaxZoomLevel);
          }
        }
